...
PHASE_SEEDS: dict[str, dict] = {
    ...
    "4e": {
        "title": "Phase 4e — Eval Engineering & Ops",
        "icon": "📐",
        "status": "complete",
        "concepts": [
            "pass@k = 1 − (1−p)^k where p is the single-run pass rate: it measures the probability that at least one of k independent agent runs produces a correct answer",
            "Why pass@k matters more than pass@1: an agent with p=0.3 looks weak at pass@1=30% but pass@5=83% — best-of-k hides real variance and single-run evals can give false confidence",
            "Deterministic eval: fact-checking against required strings/patterns with zero ambiguity — no LLM needed, runs in CI, fastest feedback loop; best for facts, numbers, schema compliance",
            "LLM-as-judge at scale: the same judge from Phase 4c run in batch over a golden dataset; evaluates quality dimensions (correctness, groundedness, coherence) that deterministic checks cannot capture",
            "The eval ops flywheel: every production failure becomes a regression test — the dataset only grows, so the quality bar rises automatically each sprint without manual curation effort",
            "CI eval gate: eval runs on every pull request; a score below baseline blocks the merge — this prevents regressions from reaching production without requiring human review of every change",
            "majority@k / voting: run k times with temperature > 0 and select the majority answer — more robust than single-sample for open-ended generation where outputs naturally vary",
            "EU AI Act Art. 9 and Art. 15: high-risk AI systems require a documented risk management system with ongoing accuracy and robustness testing; eval ops provides the technical evidence",
            "NIST AI RMF MEASURE function (2.5, 2.6): requires repeatable quantitative eval procedures and documentation that results are used to improve the system — eval ops satisfies both",
            "ISO/IEC 42001 Clause 9.1: the AI management system standard requires monitoring and measurement of AI system performance with documented procedures — the eval dataset and CI gate serve as evidence",
            "Eval dataset versioning: storing dataset version alongside eval results is required for compliance audit trails — you must be able to reproduce any past eval result given the version",
            "Langfuse vs LangSmith for eval ops: both provide Dataset APIs for golden examples and scoring APIs for eval results; Langfuse is open-source/self-hostable; LangSmith is native to LangChain/LangGraph",
        ],
        "snippets": [
            "# pass@k — unbiased single-sample estimator:\np = n_correct / n_total\npass_at_k = 1.0 - (1.0 - p) ** k  # P(at least one of k passes)",
            "# Deterministic eval — contains match:\ndef det_eval(response, facts):\n    hits = [f for f in facts if f.lower() in response.lower()]\n    return {'passed': len(hits) == len(facts), 'score': len(hits)/len(facts)}",
            "# Eval ops loop — capture production failure as regression test:\ndef capture_failure(question, expected_facts):\n    GOLDEN.append({'id': f'REG_{timestamp}', 'question': question, 'facts': expected_facts})\n    # next CI eval run will measure whether the fix resolved it",
        ],
    },
    "5": {
    ...
    "10b2": {
        "title": "Phase 10b2 — Graph Memory & Persistence",
        "icon": "🧠",
        "status": "complete",
        "concepts": [
            "A graph checkpointer is the framework equivalent of the manual history list in Phase 1b: both store every turn, but the checkpointer is thread-scoped, automatic, and swappable without changing any agent logic",
            "Thread ID scoping is how one deployed agent serves thousands of users concurrently: same thread_id loads the same state; different thread_id starts fresh — no agent logic changes required",
            "The checkpointer hierarchy is swap-by-import: in-memory for development, SQLite for zero-infra local production, Redis for distributed systems, Postgres for enterprise — all share the same API",
            "Session memory (checkpointer) and cross-session memory (memory store) solve different problems: the checkpointer stores one conversation; the memory store stores facts about a user across all conversations",
            "SQLite-backed persistence is the sweet spot for single-server production: zero infrastructure beyond a file, survives process restarts, same API as the in-memory alternative",
            "Conversation summary memory trades information completeness for token efficiency: earlier turns are compressed into a summary, keeping context window usage roughly constant as conversations grow",
            "A back-edge in a graph (node A → node B → back to node A) and a checkpointer together enable recoverable loops: if the process crashes mid-loop, the next invocation reloads state and resumes from the last checkpoint",
            "The Memory Store namespace tuple convention (namespace, user_id) mirrors how Phase 5b ChromaDB used collections per user — same concept, standardised API",
            "ConversationBufferMemory preserves every message exactly; ConversationSummaryMemory preserves meaning at lower token cost — choose based on whether exact wording matters downstream",
            "The key failure mode of in-memory checkpointers in production: process restart wipes all state, making every conversation restart from scratch — use a persistent backend for anything beyond demos",
        ],
        "snippets": [
            "# Swap checkpointer without changing agent logic\n# Dev:  checkpointer=MemorySaver()\n# Prod: checkpointer=SqliteSaver.from_conn_string('prod.db')\nagent = create_react_agent(llm, tools, checkpointer=checkpointer)",
            "# Thread ID scoping — two users, one agent\ncfg_alice = {'configurable': {'thread_id': 'user-alice'}}\ncfg_bob   = {'configurable': {'thread_id': 'user-bob'}}\n# Each config → independent state; no cross-contamination",
            "# Cross-session Memory Store\nstore.put(('user', 'alice'), 'profile', {'pref': 'concise answers'})\n# Retrieved in node: profile = store.get(('user', 'alice'), 'profile').value",
        ],
    },
    "10b3": {
        "title": "Phase 10b3 — Graph Tools & Security",
        "icon": "🔧",
        "status": "complete",
        "concepts": [
            "A tool decorator that reads type hints and docstrings to auto-generate a JSON schema replaces the manual schema definition in Phase 1c — the underlying call is identical, the boilerplate is removed",
            "A dispatch node that reads the last message's tool calls and executes them automatically replaces the manual Phase 1c loop (parse function_calls → call function → append result to history)",
            "The Guard Agent sits between the reasoning node and the dispatch node, checking whether the intended action is safe before execution — it operates at the tool-call layer, not the input layer like Phase 4a Guardrails",
            "Guard Agent vs Guardrails (Phase 4a): Guardrails check whether the user's message is safe; the Guard Agent checks whether the tool call the agent is about to make is safe — both are needed, at different layers",
            "HITL with an interrupt primitive pauses the graph at a named node, exiting the current invocation and returning control to the caller — the state is checkpointed, so resuming with the human's decision continues from exactly that point",
            "Resuming a paused graph with a human decision via a resume command is architecturally cleaner than the Phase 4b approach of re-entering a while-loop: no re-execution of completed steps, no session state flags",
            "MCP adapter libraries wrap any MCP-compatible server as a tool that the dispatch node can call — the same adapter pattern as Phase 6b, but now integrated as a standard tool type rather than a custom class",
            "Structured output enforces a typed schema on every LLM response: the model returns a validated object, not a string, eliminating the fragile regex parsing used in Phase 4c and Phase 4d",
            "The audit trail produced by auto-tracing covers every tool call: name, arguments, result, and timing — this is the tool-level visibility that Phase 7a TraceCollector captured manually",
            "High-risk tools (send_email, delete_record, payment_authorise) should always have a guard layer before dispatch; low-risk read-only tools (get_weather, lookup_price) can bypass the guard for latency efficiency",
        ],
        "snippets": [
            "# Tool decorator — schema from type hints and docstring\n@tool\ndef get_weather(city: str) -> str:\n    'Get current weather for a city.'  # ← becomes description\n    ...",
            "# HITL interrupt and resume\nhuman_decision = interrupt('Approve email send? yes/no')\n# Graph pauses here. Caller resumes with:\ngraph.invoke(Command(resume='yes'), config)",
            "# Guard before dispatch\ndef guard_node(state):\n    tool_name = state['messages'][-1].tool_calls[0]['name']\n    if tool_name in HIGH_RISK_TOOLS:\n        state['requires_approval'] = True\n    return {}",
        ],
    },
    "10b4": {
        "title": "Phase 10b4 — Graph Platform & Production",
        "icon": "🚀",
        "status": "complete",
        "concepts": [
            "Any compiled graph can render itself as a visual diagram showing nodes, edges, conditional branches, and back-edges — this is the fastest way to verify graph topology before running it",
            "The agent loop (Perceive → Reason → Act → Evaluate → Persist) maps directly to graph nodes: the checkpointer handles Persist automatically, making durable execution the default rather than the exception",
            "Andrew Ng's four agentic patterns correspond to specific graph structures: Reflection is a back-edge from evaluate to generate, Tool Use is an agent-tools cycle, Planning is a dedicated plan node, Multi-Agent is a subgraph or delegate node",
            "Streaming with a values mode returns the full state after each node completes — learners can watch each step of the agent loop execute in real time, making the abstract loop concrete",
            "Streaming with an updates mode returns only the state delta per node — more efficient for production, where full state replay is unnecessary and costly",
            "Durable execution with a persistent checkpointer means a process crash mid-loop is recoverable: the next invocation with the same thread ID picks up from the last completed node, not from the beginning",
            "The graph visualisation tool is also a debugging aid: a back-edge that creates an unintended infinite loop is immediately visible in the rendered diagram, without needing to run the agent",
            "Local Studio UI provides time-travel debugging: click any past checkpoint in the run history, modify state, and replay from that point — equivalent to git checkout for agent execution",
            "A FastAPI wrapper around a compiled graph exposes it as a REST API: one endpoint per operation, thread ID in the request body for session routing, streaming responses via Server-Sent Events",
            "Production readiness is not a checklist item added at the end — persistence, HITL, guard nodes, and tracing should be designed into the graph from the first node, not retrofitted after the core logic is built",
        ],
        "snippets": [
            "# Render any compiled graph as a PNG\npng = graph.get_graph().draw_mermaid_png()\nst.image(png, caption='Compiled graph structure')",
            ...
            "# Durable execution: same thread_id resumes from last checkpoint\ncfg = {'configurable': {'thread_id': 'user-123'}}\ngraph.invoke(input, cfg)  # process crash here\ngraph.invoke(input, cfg)  # resumes — SqliteSaver loaded the state",
        ],
    },
    "10e": {
    ...
    "7d": {
        "title": "Phase 7d — Stateful Task Management",
        "icon": "📋",
        "status": "complete",
        "concepts": [
            "Stateless agents lose task progress on server restart or failure — stateful task management checkpoints state after every step so tasks can resume from where they stopped",
            "Task lifecycle states: SUBMITTED → QUEUED → RUNNING → COMPLETED / FAILED / DEAD_LETTER / CANCELLED — every production agent task must track these explicitly",
            "Checkpoint/resume pattern: after each completed step, persist the step name and result to a durable store (Redis, DB); on restart, reload state and skip already-completed steps",
            "Idempotency key: a unique identifier for a task submission — if the same key is submitted twice, the second call returns the cached result without re-executing the task",
            "Dead-letter queue: tasks that fail after N retries are moved to a separate store for human review rather than blocking the main queue indefinitely",
            "Task queue decoupling: submitting a task and executing it are separate operations — the queue buffers submissions and workers pull tasks independently, enabling horizontal scaling",
            "Why session state is not enough for production: browser-based state (st.session_state) is lost on page refresh or server restart; production tasks need durable persistence",
            "Retry with backoff: failed tasks should be retried with exponential backoff — immediate retries on a flaky external service cause thundering herd and exhaust rate limits",
            "At-least-once vs exactly-once: task queues typically guarantee at-least-once delivery; idempotency keys ensure the agent's effect happens exactly once even if the task runs twice",
        ],
        "snippets": [
            "# Checkpoint after each step — skip on resume:\nfor step in steps:\n    if step.name in state['completed_steps']: continue  # already done\n    result = run_step(step, state['results'])\n    state['completed_steps'].append(step.name)\n    checkpoint.save(state)",
            "# Idempotency check — return cached result:\nexisting = checkpoint.load(idempotency_key)\nif existing and existing['status'] == 'completed':\n    return {'cached': True, 'result': existing['results']}",
            "# Dead-letter after max retries:\nstate['retry_count'] += 1\nif state['retry_count'] >= MAX_RETRIES:\n    state['status'] = 'dead_letter'\nelse:\n    state['status'] = 'failed'  # will be retried",
        ],
    },
    "8d": {
        "title": "Phase 8d — Multimodal Agents",
        "icon": "👁️",
        "status": "complete",
        "concepts": [
            "Multimodal agents perceive images alongside text — the model processes both simultaneously in the same forward pass, not sequentially",
            "Image input vs OCR: OCR extracts text but loses layout, colour, and spatial relationships; a multimodal model reads an image directly and understands that a red cell means warning, a downward arrow means declining trend",
            "The contents list: multimodal calls pass a list of Part objects (image bytes + text) instead of a plain string — Part.from_bytes(data, mime_type) and Part.from_text(question)",
            "Visual RAG: embed image descriptions semantically, retrieve by cosine similarity, send the image to the model along with the question — extends Phase 5a RAG to image-heavy knowledge bases",
            "Document parsing with vision: PDFs with embedded charts, tables, or diagrams return garbled text via text extraction; sending the page as an image preserves layout and structure",
            "Image tokens have a cost: a 1024×1024 image consumes approximately 258 tokens at standard rates — factor this into budget calculations for high-volume image processing",
            "Multimodal agents extend Computer Use (Phase 8c): before acting on a screenshot, the agent describes what it sees — reads error messages, button labels, and form states accurately",
            "Structured extraction from images: combining image input with JSON response mode extracts tables, key-value pairs, and structured data from photos of documents, receipts, or forms",
            "Privacy consideration: images sent to the API may contain PII — ensure compliance requirements are met before sending photos of documents, identity cards, or faces",
        ],
        "snippets": [
            "# Multimodal call — image + text in one request:\nresponse = client.generate_content(\n    model=MODEL,\n    contents=[\n        Part.from_bytes(data=image_bytes, mime_type='image/jpeg'),\n        Part.from_text('What does this chart show?'),\n    ]\n)",
            "# Structured extraction from document image:\nresponse = client.generate_content(\n    model=MODEL,\n    contents=[Part.from_bytes(doc_bytes, 'image/jpeg'), Part.from_text('Extract JSON: {title, date, totals}')],\n    config=GenerateContentConfig(response_mime_type='application/json'),\n)",
            "# Visual Q&A agent pattern:\nfor image, question in tasks:\n    answer = ask_about_image(image_bytes=image, mime_type='image/jpeg', question=question)\n    results.append({'question': question, 'answer': answer})",
        ],
    },
    "8e": {
        "title": "Phase 8e — Enterprise Event-Driven Patterns",
        "icon": "⚡",
        "status": "complete",
        "concepts": [
            "Event-driven architecture decouples agent triggering from agent execution — a business system publishes an event; the agent consumes it asynchronously without either knowing about the other",
            "Agent as event consumer: agent reads from an input topic (e.g. transactions.completed), processes the event, publishes results to an output topic (e.g. fraud.verdicts) — fully asynchronous",
            "Fan-out pattern: one event triggers multiple independent agents simultaneously, each consuming from the same topic and publishing to separate output topics; an aggregator reads all outputs",
            "Dead-letter queue in event-driven systems: events that fail processing after N retries are published to a DLQ topic rather than blocking the main consumer — enables human review without pipeline stall",
            "Event replay: because Kafka retains all events, a new agent version can be deployed and re-run over historical events from offset 0 — useful for debugging, auditing, and model upgrades",
            "At-least-once delivery: message brokers guarantee events are delivered at least once but may deliver duplicates — agents must be idempotent (same event processed twice produces the same result)",
            "Async request-response over events: client publishes a request event with a correlation_id; agent processes and publishes a response event with the same correlation_id; client polls the response topic",
            "Why event-driven beats synchronous for agents at scale: queue buffers spikes (no dropped requests), failures are isolated (bad event goes to DLQ, not lost), and consumers scale independently of producers",
            "Kafka consumer group: multiple agent instances form a consumer group — Kafka distributes partitions among them, achieving parallel processing without duplicate processing of the same event",
        ],
        "snippets": [
            "# Agent consumer loop (Kafka):\nfor message in consumer:  # blocks until event arrives\n    event = message.value\n    result = agent.process(event)\n    producer.send('output.topic', value=result)\n    consumer.commit()  # acknowledge after success",
            "# Fan-out: one event, multiple agents:\n# All three consumer groups read the same topic independently:\n# fraud-agent-group, compliance-agent-group, risk-agent-group\n# Each publishes to its own output topic",
            "# Idempotency in event processing:\nif already_processed(event['id']):\n    return  # deduplicate — safe to skip\nresult = process(event)\nmark_processed(event['id'])\npublish(result)",
        ],
    },
}
