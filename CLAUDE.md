...
utils/
  ...
  _diagram_phase10.py   # Phase 10 diagram functions still on matplotlib:
                        #   diagram_google_adk, diagram_lang_arch_map
                        #   (2 functions — the rest were replaced by static arch-diagram-skill
                        #   JPEGs in docs/images/arch_*.jpg, see Phase 10 Technical Notes below)
...
- **Phase 10 diagrams — two pipelines now coexist:**
  - **Matplotlib (legacy)**: `utils/_diagram_phase10.py` still holds 2 functions — `diagram_google_adk`, `diagram_lang_arch_map` — injected into `utils/diagrams.py` via the `_ph10_diagrams()` factory tuple. When adding/removing one, update the `return (...)` tuple AND the unpacking assignment in `diagrams.py` in lockstep, then restart Streamlit (module-cache gotcha — edits to `_diagram_phase10.py` are not hot-reloaded).
  - **Static arch-diagram-skill JPEGs (current standard for new/replaced diagrams)**: generated via `/ckm:arch-diagram` (table-only HTML → `wkhtmltoimage` → JPEG), copied into `docs/images/arch_*.jpg`, loaded directly with `st.image("docs/images/arch_xxx.jpg", ...)`. No Python function, no Streamlit restart needed for content edits — just regenerate the JPEG and it's picked up on next page load. This pipeline replaced the matplotlib diagrams for: `arch_langgraph_workflows`, `arch_langgraph_agents`, `arch_langgraph_memory`, `arch_langgraph_tools_security`, `arch_langgraph_platform`, `arch_langsmith`, `arch_langchain`, `arch_framework_compare` (pages 06a, 06b, 06a1, 06a2, 06a3, 06c, 06d, 06f). The skill's editable HTML sources live in `./output/` (kept as source-of-truth for future iteration via the skill's "Add a layer / Change a color" prompts — gitignored or untracked, not part of the app).
  - **wkhtmltoimage + Arial font cannot render emoji/dingbat glyphs** (⚠ 💡 🔒 ✅ 📋 render as empty boxes). Use Arial-safe Unicode instead: `&#9650;` ▲, `&#9679;` ●, `&#9632;` ■, `&#10003;` ✓, `&#9656;` ▸, `&#8594;` →.
  - **Font sizing**: the skill's golden-prompt defaults (9–11px body text) render small at 1500px width. Current diagrams use a ~30% scale-up (body text ~12.5px, layer names ~14px, titles ~28px) — match this when generating new ones.
